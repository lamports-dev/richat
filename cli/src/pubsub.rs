...
#[derive(Debug, Args)]
pub struct ArgsAppPubSub {
    /// Solana PubSub endpoint
    #[clap(default_value_t = String::from("ws://127.0.0.1:8000"))]
    endpoint: String,

    #[command(subcommand)]
    action: ArgsAppPubSubAction,
}

impl ArgsAppPubSub {
    pub async fn run(self) -> anyhow::Result<()> {
        let client = PubsubClient::new(&self.endpoint).await?;
        match self.action {
            ArgsAppPubSubAction::Subscribe {
                action,
                commitment,
                stats,
            } => {
                            ...
                            .program_subscribe(
                                ...
                                Some(RpcProgramAccountsConfig {
                                    ...
                                    account_config: RpcAccountInfoConfig {
                                        ...
                                        data_slice: ArgsActionSubscribe::parse_data_slice(
                                            &data_slice,
                                        )?,
                                        commitment: Some(commitment.into()),
                                        min_context_slot: None,
            ...
            ArgsAppPubSubAction::GetVersion => {
                let (SubscribeResponse::GetVersion(version), mut stream) =
                    subscribe_plain(&self.endpoint, "getVersion", json!([])).await?
                else {
                    anyhow::bail!("invalid response");
                };
                info!(
            ...
            ArgsAppPubSubAction::GetVersionRichat => {
                let (SubscribeResponse::GetVersionRichat(version), mut stream) =
                    subscribe_plain(&self.endpoint, "getVersionRichat", json!([])).await?
                else {
                    anyhow::bail!("invalid response");
                };
...
pub enum ArgsUiAccountEncoding {
    Binary,
    Base58,
    Base64,
    JsonParsed,
...
#[derive(Debug, Clone, Copy, ValueEnum)]
pub enum ArgsUiTransactionEncoding {
    Binary,
    Base64,
    Base58,
    Json,
...
#[derive(Debug, Clone, Copy, ValueEnum)]
pub enum ArgsTransactionDetails {
    Full,
    Signatures,
    None,
    Accounts,
...
pub enum ArgsActionSubscribeCommitment {
    Processed,
    Confirmed,
    #[default]
    Finalized,
