...
impl ArgsAppStream {
    ...
    pub async fn run(self) -> anyhow::Result<()> {
        ...
        let pb_txs_st =
            crate_progress_bar(&pb_multi, ProgressBarTpl::Msg("transactions statuses"))?;
...
fn create_pretty_account(account: SubscribeUpdateAccountInfo) -> anyhow::Result<Value> {
    Ok(json!({
        ...
        "txnSignature": account.txn_signature.map(|sig| Signature::try_from(sig).unwrap().to_string()),
    }))
}
